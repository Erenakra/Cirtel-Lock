package dev.cirtellock.applock.services
...
import dev.cirtellock.applock.core.broadcast.DeviceAdmin
...
import dev.cirtellock.applock.features.lockscreen.ui.LockScreenOverlayManager
import dev.cirtellock.applock.services.AppLockConstants.ACCESSIBILITY_SETTINGS_CLASSES
import dev.cirtellock.applock.services.AppLockConstants.EXCLUDED_APPS
...
class AppLockAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        private const val APP_PACKAGE_PREFIX = "dev.com.cirtellock.applock"
    ...
    private fun isValidPackageForLocking(packageName: String): Boolean {
        ...
        return !((packageName == APP_PACKAGE_PREFIX) ||
    ...
    private fun checkForDeviceAdminDeactivation(event: AccessibilityEvent) {
        ...
        //val isOurAppVisible = findNodeWithTextContaining(rootNode, "Cirtel Lock") != null ||
    ...
    private fun isDeactivationAttempt(event: AccessibilityEvent): Boolean {
        val isAccessibilitySettings = event.className in ACCESSIBILITY_SETTINGS_CLASSES &&
                event.text.any { it.contains("Cirtel Lock") }
        ...
        val isAlertDialog =
            event.packageName == "com.google.android.packageinstaller" && event.className == "android.app.AlertDialog" && event.text.toString()
                .lowercase().contains("Cirtel Lock")
    ...
    private fun findNodeWithTextContaining(
        ...
        return try {
            ...
            for (i in 0 until node.childCount) {
                ...
                findNodeWithTextContaining(child, text)?.let { return it }
            }
            null
        } catch (e: Exception) {
        ...
        try {
            ...
            when (appLockRepository.getBackendImplementation()) {
                BackendImplementation.USAGE_STATS -> {
                    ...
                    val intent = Intent(this, UsageLockService::class.java)
